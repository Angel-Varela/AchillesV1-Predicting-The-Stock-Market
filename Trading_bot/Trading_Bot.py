...
if mt5.initialize():
    pass
    if not mt5.initialize(login=52061328, server="ICMarketsSC-Demo",password="JZYado3C!D9NWL"):
            print("initialize() failed, error code =",mt5.last_error())
...
class TradingBot():
    def get_df(self):
        ...
        dataframe = pd.read_csv(r"C:\Users\Angel\OneDrive\Escritorio\Vs Code\Achilles\Gold_Predictions.csv", names=columns_names, skiprows=1)

        #Right here we can  use the date colum on the dataframe in order to check the row and find the nearest 10 rows

        now_time = datetime.now().replace(microsecond=0, second=1) #We'll change seconds depending on last second on the date column dataframe
    ...
    def inner_loop(self): 
        ...
        print("Sentiment 4:", usd_sentiment)
    ...
    def outer_loop(self):
        ...
        while True:
            if time.time() - start_time >= 900: #GET THE NEWS EACH 15 MINUTES
                start_time = time.time() #Update the start time to run each 20 minutes

            
            nearest_rows, _, _, _ = self.get_df() #Take the Nearest Rows
            qty = self.positional_sizing() #Take the Volume we will invest based on Positional Sizing

            #Take the now's time
            now_time = datetime.now().replace(microsecond=0, second=1) #We'll change seconds depending on the dataframe
